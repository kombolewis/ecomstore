...
class Validate {
  ...
  public function check($source, $items=[]) {
    ...
    foreach($items as $item => $rules) {
      /**
       * item is array_keys(username,password)
       * display is the formatted field name
       */
      ...
      foreach($rules as $rule => $rule_value) {
        ...
        /**
         * rule is the array key for the inner array(required,display)
         * rule_value is the array value for the inner array
         * value is user input
         */
        ...
        if($rule == 'required' && empty($value)) {
          ...
          switch ($rule) {
            ...
            case 'is_numeric':
              if(!is_numeric($value)) {
                $this->addError(["{$display} has to be a number. Please use a numeric value", $value]);
              }
              break;
